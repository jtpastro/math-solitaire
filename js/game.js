...
// 0. DECLARE VARS   
   ...
   suits['diamonds'] = buildSuit(sequence, 'diamond');
   suits['clubs'] = buildSuit(sequence, 'club');
   suits['hearts'] = buildSuit(sequence, 'heart');
...
// ### FUNCTIONS ###
   ...
   function revalueRanks(){
      var template = d.querySelectorAll('.template li');
      var cardRanks = [];
      for(var i=0;i<template.length;i++){
         template[i].dataset.rank=sequence[i];
         
         cardRanks = template[i].querySelectorAll('.rank');
         cardRanks[0].innerText = sequence[i];
         cardRanks[1].innerText = sequence[i];
      }
   }

   //build suit
      function buildSuit(sequence, suit){
         var seqSuit = [];
         for(var i=0;i<sequence.length;i++){
            seqSuit.push([sequence[i], suit]);
         }
         return seqSuit;
      }
         ...
         if (clicks === 1 && event.type === 'click') {
            clickTimer = setTimeout(function() {
               ...
               // if no move is in progress
                  ...
                  // if ace is selected
                  if (rank == sequence[0]) {
                     console.log('Menor valor selecionado.');
                     bindClick('#fnd #'+suit+'s.pile[data-empty="true"]');
                  }
                  if (rank == sequence[sequence.length-1]) {
                     console.log('Maior valor selecionado.');
   ...
   // validate adjacent
      function validateAdjacent(dRank, sRank){
         return sequence.indexOf(dRank) - sequence.indexOf(sRank);
      }
   ...
   // validate move
      function validateMove(selected, dest) {
         ...
         if (dest.constructor === Array) {
            console.log('Destination appears to be a card');
            ...
            if (['spades','hearts','diamonds','clubs'].indexOf(dPile) >= 0) {
               ...
               if (validateAdjacent(dRank, sRank) != -1) {
         ...
         if (['spades','hearts','diamonds','clubs'].indexOf(dest) >= 0) {
            ...
            else if ( sRank === sequence[0] ) {
               console.log('Valid Move');
               return true;
            }
            // if rank isn't in sequence then return false
            else if ( sequence.indexOf(dRank) - sequence.indexOf(sRank) !== 1 ) {
               console.log('Rank sequence invalid');
               return false;
            }
            // else return true
